...
import { TestSessionDemo } from "../components/TestSessionDemo";
...
export function AdminTestControl() {
  const { activeTestControl } = useAdminStore();
  const [session, setSession] = useState<TestSession>({
    id: `session-${activeTestControl?.id}`,
  ...
  const handleStopSection = async () => {
    ...
    try {
      ...
      // const data = await response.json();
      // console.log(
      //   `Section ${session.currentSection} stopped successfully:`,
      //   data
      // );
      // setSession((prev) => ({
      //   ...prev,
  ...
  const isSectionCompleted = (sectionName: SectionName) => {
    return (
      session.completedStudents.get(sectionName)?.size ===
      activeTestControl?.students.length
    );
  };
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-gray-600">
                Test Date:{" "}
                {new Date(activeTestControl.testDate!).toLocaleDateString()}
              </p>
            </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-900 mt-1">
                    {session.connectedStudents.size} /{" "}
                    {activeTestControl.students.length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-900 mt-1">
                    {activeTestControl.attemptsAllocated}
                  </p>
                </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${
                            ((session.completedStudents.get(section.name)
                              ?.size || 0) /
                              activeTestControl.students.length) *
                            100
                          }%`,

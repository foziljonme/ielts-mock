...
export class ExamsService {
    ...
    return await this.prismaService.$transaction(async (tx) => {
      ...
      const examSeat = await tx.examSeat.create({
        data: {
          ...
          label: 'Computer 1',
          accessCode,
          assignedStudentName: 'John Doe',
          assignedStudentId: '123456',
        },
      });

      const seatAttemptPayload = {
        seatId: examSeat.id,
        scheduleId: examSchedule.id,
        status: AttemptStatus.IN_PROGRESS,
      };

      const seatAttempt = await tx.seatAttempt.create({
        data: seatAttemptPayload,
      });
  ...
  async getAllQuestions(examId: string) {
    const exam = await this.prismaService.examSchedule.findUnique({
      where: { id: examId },
    });

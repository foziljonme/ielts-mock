...
type View = 'student-login' | 'admin-login' | 'staff-login' | 'waiting-room' | 'section-instructions' | 'section-selection' | 'test' | 'completion' | 'admin' | 'staff';
...
export default function App() {
  ...
  const [currentSectionName, setCurrentSectionName] = useState<SectionName | null>(null);
  const [adminStartedSection, setAdminStartedSection] = useState<SectionName | null>(null);

  const handleStudentLogin = (code: string) => {
    setAccessCode(code);
    // In real implementation, check if test is lab-controlled or self-paced
    // For lab-controlled tests, go to waiting room
  ...
  const handleSectionStart = (section: SectionName) => {
    // Called when admin starts a section
    setCurrentSectionName(section);
    setCurrentView('section-instructions');
  };

  const handleInstructionsComplete = () => {
    // Move to actual test after instructions
    // For now, go to section selection for demo
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {currentView === 'waiting-room' && (
        <StudentWaitingRoom
          ...
          currentSection={adminStartedSection}
          onSectionStart={handleSectionStart}
        />
      )}
      
      {currentView === 'section-instructions' && currentSectionName && (
        <SectionInstructions
          section={currentSectionName}
          duration={currentSectionName === 'listening' ? 30 : currentSectionName === 'speaking' ? 15 : 60}
          onReady={handleInstructionsComplete}
        />
      )}
